...
export class Runner {
  ...
  async executeTaskInstance(taskInstance, taskPromises, limit, serialLimit) {
    ...
    return limiter(async () => {
      ...
      try {
        ...
        // Execute internal functions (__call, __input, __input_password, etc.)
        ...
        if (!this.quiet) {
          console.error(chalk.red(`[${signature}] Failed: ${error.message}`));
        }
  ...
  printSummary(startTime) {
    ...
    this.outputManager.printSummary(this.completed, this.failed, duration);
  }
  ...
  async executeInternalFunction(func, variables, signature, taskPromises, limit, serialLimit) {
    ...
    switch (functionName) {
      case 'input':
        return await this.handleInputFunction(func, variables);
      case 'input_password':
        return await this.handleInputPasswordFunction(func, variables);
      case 'input_select':
        return await this.handleInputSelectFunction(func, variables);
      case 'input_confirm':
        return await this.handleInputConfirmFunction(func, variables);
      case 'call':
        return await this.handleCallFunction(func, variables, taskPromises, limit, serialLimit);
      default:
        if (!this.quiet) {
          console.warn(chalk.yellow(`Warning: Unknown internal function __${functionName}`));
        }
        return null;
    }
  }

  async handleInputFunction(func, variables) {
  ...
  async handleInputPasswordFunction(func, variables) {
  ...
  async handleInputSelectFunction(func, variables) {
  ...
  async handleInputConfirmFunction(func, variables) {
  ...
  async handleCallFunction(func, variables, taskPromises, limit, serialLimit) {
    ...
    return await this.executeCall(call, variables, taskPromises, limit, serialLimit);
  }
  ...
  parseParameter(param, variables) {
    ...
    if (param.type === 'array') {
      return param.value; // Return the array directly
    }
