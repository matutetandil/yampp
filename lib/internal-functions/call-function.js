...
export class CallFunction extends BaseInternalFunction {
  async execute(func, variables, signature, taskPromises, limit, serialLimit) {
    ...
    if (func.params.length > 1) {
      if (func.params[1].type === 'params') {
        // Parameters are in parentheses format from parser: __call task(param1, param2)
        params = func.params[1].value || [];
      } else {
        // Parameters are individual arguments from intercept: __call task param1 param2
        params = func.params.slice(1); // All params after task name
      }
    }
