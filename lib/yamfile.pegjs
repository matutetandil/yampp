...
Task
  = mods:Modifiers? _ name:Identifier _ params:Parameters? _ clauses:Clauses? _ "{" _ body:TaskBody _ "}" _ {
      const { commands, localVars, localConsts, calls, internalFunctions } = body || { commands: [], localVars: [], localConsts: [], calls: [], internalFunctions: [] };
...
TaskBody
  = lines:TaskLine* {
      ...
      return { commands, localVars, localConsts, calls, inputs, internalFunctions };
    }

TaskLine
  = _ line:(LocalVariable / InternalFunction / Assignment / Command / Comment) _ Newline? { return line; }
...
Command
  = !("const" / "var" / "__" / "}") chars:CommandCharacters+ {
      return {
        type: 'command',
