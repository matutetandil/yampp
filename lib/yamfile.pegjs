...

{
  ...
  function makeTask(modifiers, name, params, clauses, commands, location) {
    return {
      ...
      optionalDependencies: clauses?.optionalDependencies || [],
  ...
  function makeDependencyWithParams(name, params, optional = false) {
    return {
      name: name,
      parameters: params || [],
      optional: optional
...
Modifier
  ...
  / "serial" !([a-zA-Z0-9_]) { return 'serial'; }
  / "critical" !([a-zA-Z0-9_]) { return 'critical'; }
...
Clauses
  = needs:NeedsClause _ watches:WatchesClause? {
      return {
        ...
        optionalDependencies: needs.optionalDependencies || [],
...
NeedsClause
  = "needs" _ deps:DependencyList {
      ...
      for (const dep of deps) {
        if (dep.optional) {
          optionalDependencies.push(dep.name);
        } else {
      ...
      return { dependencies, optionalDependencies, dependencyParams };
    }
...
DependencyWithParams
  = optional:"!"? _ name:Identifier _ params:CallParameters? {
      return makeDependencyWithParams(name, params, !!optional);
    }
