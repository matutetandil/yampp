...
export class Validator {
  ...
  validateTaskInputs(task) {
    ...
    if (!task.hasModifier('serial')) {
      this.addError(
        `Task '${task.name}' uses input prompts but is not marked as 'serial'. Input prompts require serial execution to prevent concurrent prompts.`,
        task.lineNumber
      );
    }
