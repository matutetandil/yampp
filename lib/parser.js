...
export class Parser {
  constructor() {
    ...
    this.taskPattern = /(?:((?:\w+\s+)*))(\w+)(?:\s*\(([^)]*)\))?\s*(?:needs\s+([^{]+))?\s*(?:watches\s+([^{]+))?\s*\{([^}]*)\}/gms;
  ...
  parse(content) {
    ...
    while ((match = this.taskPattern.exec(cleanContent)) !== null) {
      const [fullMatch, modifiersStr, taskName, parametersStr, dependenciesStr, watchedFilesStr, commandsBlock] = match;
      
      // Parse modifiers (space-separated)
      const modifiers = modifiersStr 
        ? modifiersStr.trim().split(/\s+/).filter(Boolean)
        : [];
      ...
      // Parse dependencies with their parameters
      const { dependencies, dependencyParams } = this.parseDependenciesWithParams(dependenciesStr);
      
      // Parse watched files
      const watchedFiles = this.parseWatchedFiles(watchedFilesStr);
