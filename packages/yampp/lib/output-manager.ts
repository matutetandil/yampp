...
import { OutputManagerOptions } from './output/interfaces/output-manager-options.interface.js';
import { LineInfo } from './output/interfaces/line-info.interface.js';
import { TaskBuffer } from './tasks/interfaces/task-buffer.interface.js';
import { ICompleteOutputManager } from './core/types/complete-output-manager.interface.js';

/**
 * OutputManager implementation
 * Implements segregated interfaces following Interface Segregation Principle
 */
export class OutputManager implements ICompleteOutputManager {
  private readonly quiet: boolean;
  private readonly verbose: boolean;
  private readonly ugly: boolean;
  private readonly isInteractive: boolean;
  private readonly taskBuffers: Map<string, TaskBuffer>;
  private readonly taskOrder: string[];
  private readonly maxLinesPerTask: number = 10;
  private currentRow: number = 0;
  private readonly initialRow: number = 0;
  private readonly blockPositions: Map<string, number>;
  private readonly collapsedTasks: Set<string>;
  private initialized: boolean = false;
  private renderTimer?: NodeJS.Timeout;
  private lastRenderTime: number = 0;
  private readonly renderInterval: number = 100;
  ...
  public startTask(taskId: string, taskName: string): void {
  ...
  public addOutput(taskId: string, line: string, isError: boolean = false): void {
    if (this.quiet) return;
  ...
  private renderTaskBlock(taskId: string, newLine: string | null = null, isError: boolean = false): void {
    const buffer = this.taskBuffers.get(taskId);
    if (!buffer) return;
  ...
  private updateTaskBlock(taskId: string): void {
    const buffer = this.taskBuffers.get(taskId);
  ...
  public printSummary(completed: Set<string>, failed: Set<string>, duration: string): void {
    if (this.quiet) return;
  ...
  public log(message: string, color = chalk.white): void {
    if (!this.quiet) {
