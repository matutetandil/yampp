...
export class AsciiGraphFormatter extends BaseGraphFormatter {
  ...
  private formatSingleTaskAscii(taskName: string): void {
    ...
    if (dependents.length > 0) {
      console.log('         │');
      console.log('         ▼');
      dependents.forEach((dependent: string, index: number) => {
  ...
  private formatFullGraphAscii(): void {
    ...
    levels.forEach((level, levelIndex) => {
      ...
      if (levelIndex < levels.length - 1) {
        ...
        level.forEach((task, index) => {
          ...
          const hasConnection = levels[levelIndex + 1] ? dependents.some((d: string) => levels[levelIndex + 1]!.includes(d)) : false;
  ...
  private organizeLevels(executionOrder: string[]): string[][] {
    const levels: string[][] = [];
  ...
  private createBox(taskName: string, highlight: boolean = false): string[] {
    const task = this.tasks.get(taskName);
    const modifiers = task ? Array.from(task.getModifiers()) : [];
  ...
  private getBoxWidth(taskName: string): number {
    const task = this.tasks.get(taskName);
    const modifiers = task ? Array.from(task.getModifiers()) : [];
